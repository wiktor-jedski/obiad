...
test.describe('real-stack smoke', () => {
  test('the generated client, executed in Chromium, receives the five-item seeded response through the preview /api proxy', async ({
    ...
    expect(bundlePath, 'run through `bun run test:e2e` so the launcher builds the browser client bundle').toBeTruthy();
    ...
    const suggestions = await page.evaluate(async (): Promise<FoodSuggestionsResponse> => {
      const generatedClient = (globalThis as { __obiadGeneratedClient?: GeneratedClient }).__obiadGeneratedClient;
      if (!generatedClient) {
        throw new Error('browser client bundle did not register __obiadGeneratedClient');
      }
    ...
    // host (ARCH-016).
    expect(requestUrls.some((url) => url.includes('/api/v1/food-suggestions'))).toBe(true);
  ...
  test('the empty shell makes no startup API request and only contacts its own origin', async ({ page }) => {
    const requestUrls: string[] = [];
    page.on('request', (request) => requestUrls.push(request.url()));
