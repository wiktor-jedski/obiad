 ...
 *     with no excluded Phase 6 (Interface Language) or Phase 7 (suggestion
 *     list, selected input, result cards) region and no application API
 *     request;
...
async function assertEmptyShell(
  ...
  // No excluded Phase 6 or Phase 7 region: no Interface Language control,
  // suggestion list, selected input, result card, or result-state control.
  await expect(page.locator('button')).toHaveCount(0);
  await expect(page.locator('select')).toHaveCount(0);
  await expect(page.locator('a')).toHaveCount(0);
  await expect(page.locator('img, svg, [role="img"]')).toHaveCount(0);
  await expect(page.locator('[role="combobox"], [role="listbox"], [role="option"]')).toHaveCount(0);
  ...
  expect(await input.evaluate((element) => element.hasAttribute('autofocus'))).toBe(false);
  await expect(input).not.toBeFocused();
  expect(await page.evaluate(() => document.activeElement === document.body)).toBe(true);

  // No icon: no background image, no nested image markup, and the native
  // search appearance is removed.
  expect(await input.evaluate((element) => getComputedStyle(element).backgroundImage)).toBe('none');
  expect(await input.evaluate((element) => getComputedStyle(element).appearance)).toBe('none');
  ...
  expect(Math.abs(column.width - Math.min(vp.width, COLUMN_MAX_WIDTH_PX))).toBeLessThanOrEqual(1);
  expect(Math.abs(column.x + column.width / 2 - vp.width / 2)).toBeLessThanOrEqual(1);
  ...
  const expectedWidth = Math.min(vp.width - 2 * vp.gutterPx, FIELD_MAX_WIDTH_PX);
  ...
  expect(Math.abs(box!.y + box!.height / 2 - expectedVerticalCenter)).toBeLessThanOrEqual(1);
  ...
  console.log(`[empty-shell] review attachment ${screenshotName}: ${screenshotPath} (mirrored to ${mirror})`);
}

test.describe('empty shell', () => {
  for (const vp of VIEWPORTS) {
    test(`the empty-state Search control at ${vp.name}`, async ({ page }, testInfo) => {
