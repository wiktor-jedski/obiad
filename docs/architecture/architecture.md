...
An Ingredient contains `id`, localized `names`, one `source` URL, and `macro_profile` with per-100 g `protein`, `available_carbohydrate`, and `fat`; carbohydrate means available carbohydrate. Its Macro Profile can be all zero when its source supports this, such as for water. Its optional `density` contains a positive g/ml `value` and one `source` URL. Its optional `conversions` entries contain one sourced positive finite `quantity_g`, a nonempty `unit`, and an optional nonempty `size`; unit-and-size entries are unique exact keys. A record cannot create more than one applicable direct-gram, sourced-density-millilitre, or stored exact conversion path. Reviewed normalized source-term mappings in `defaults/ingredient-kinds.json` use a positive `schema_version` and stable Ingredient IDs; run-local overrides are invalid. A Meal contains `id`, localized `names`, one `source` URL, a positive `servings` count, `composition`, and ordered `steps`; its `composition` entries reference stable Ingredient IDs, contain a positive finite `quantity_g`, and must not duplicate an Ingredient ID. Its optional `food_family_id` references `food_families.json`. Its steps contain nonempty `instruction`, `source`, and positive integer `source_step`; `source_step` is strictly increasing. The production repository keeps raw provider downloads, source pages or text, checksums, and generated aggregates outside Git.
