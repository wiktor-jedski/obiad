...
**Flow:** Application-owned dummy catalog data provides local startup, CI, and integration data without `data/`; legacy dummy fixtures do not define production catalog eligibility. Setup runs `dbsetup` and then `catalogload` with one required catalog-file path. `catalogload` uses `OBIAD_SCHEMA_OWNER_DATABASE_URL` and the shared `dbsetup` advisory-lock key `0x0B1AD0001`. A production launcher validates and exports `data/` to a temporary aggregate, validates it against the application-owned schema, loads it transactionally, and removes the temporary aggregate. Phase 30 release verification and the Phase 31 pinned-submodule launcher own production provenance and attribution outside the aggregate. ARCH-006 reads only the loaded PostgreSQL rows. HTTP returns Food Objects, never Ingredients. Candidate similarities, rank order, and pages are derived by the backend; input calories, Matched Quantities, and scaled macronutrients are projected by the browser from canonical Macro Profiles and are never stored.
...
Fiber listens on loopback only. The POC adds no CORS mechanism, TLS, authentication, cookies, rate limiter, or third-party runtime system. The pgx pool has zero minimum and four maximum connections. Local deployment setup and integration fixtures create separate schema-owner and runtime database users before `dbsetup` runs. They remove `PUBLIC` object-creation and temporary-table privileges. Setup runs `dbsetup` and then the source-agnostic `catalogload` command with one required catalog-file path. Both commands use `OBIAD_SCHEMA_OWNER_DATABASE_URL` and advisory-lock key `0x0B1AD0001`. The variable selects the target database, not the catalog file. The file follows ARCH-013: exactly schema version `1`, at least one Food Object, zero or more Food Families, optional nonempty `imageKey`, and no provenance metadata or catalog-kind discriminator. Fiber uses the SELECT-only credential. Credentials are environment-provided and are not committed.
