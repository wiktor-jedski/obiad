...
**PostgreSQL storage:** Migration `0006_external_catalog` removes the historical seed result and leaves Food Objects and Food Families empty before the first offline load. Applied migration files remain unchanged. `nutrition_basis` is required and accepts only `g` or `ml`; no Physical State or Ingredient column is present. A Food Object stores one optional positive finite Serving value. Its generated `serving_unit` is the Nutrition Basis unit when a Serving exists and is null otherwise, so a second or mismatched Serving unit cannot be stored. Each Food Family requires localized names, and each Food Object may store one absolute source URI and one nonempty opaque image key. Catalog metadata is not stored.
